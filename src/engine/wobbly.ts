...
export class WobblyText extends TextEntity {
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    ...
    let offset =
      this.options?.align === "center" || !this.options?.align ? -w * 0.5 : 0;
    ...
    this.content.split("").forEach((letter, i) => {
      ...
      ctx.fillText(
        letter,
        this.p.x + spacing * i + this.options.shadow + offset,
        this.p.y +
          this.options.shadow +
          Math.sin(this.time * 0.005 + i * this.frequency) * this.amplitude,
      );
      ctx.fillStyle = "#fff";
