...
export class Face extends Entity {
  ...
  constructor(game: Game, options: FaceOptions) {
    ...
    this.left = new Eye(
      game,
      -50 * this.options.width,
      0,
      this.options.eyeSize,
    );
    this.right = new Eye(
      game,
      50 * this.options.width,
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    drawEllipse(
      ctx,
      { x: -65 * this.options.width - this.options.blushOffset, y: 20 },
      15 * this.options.blushSize,
      10 * this.options.blushSize,
      this.options.blush,
    );
    drawEllipse(
      ctx,
