...
export class Ship extends Flashable {
  ...
  private crew: Dude[] = [];
  private availableRoles: CrewRole[] = [
    "quartermaster",
    "cannoneer",
    "navigator",
  ];
  ...
  private pop(text: string, x: number, y: number): void {
    this.effects.add(
      new TextEntity(
        ...
        text,
        60,
        this.p.x + x * this.getDir(),
        this.p.y + y,
        1.2,
        { x: 0, y: -2.5 },
        { shadow: 4, scales: true },
      ),
    );
  }
  ...
  public hurt(amount: number): number {
    const target =
      this.dice.find((d) => d.getValue() > amount) ??
      [...this.dice].sort((a, b) => a.getValue() - b.getValue())[0];
    if (!target) return;
  ...
  public hurtDice(target: Dice, amount: number): void {
    target.mark();
    setTimeout(() => {
      this.game.pitcher.pitchFrom(
        this.player ? 0.85 : 1.21,
        this.player ? 1 : 2,
      );
      this.openMouth();
      this.flash();
  ...
  public update(tick: number, mouse: Mouse): void {
    ...
    [...this.dice, ...this.tempDice].forEach((d) =>
      d.update(tick, this.offsetMouse(mouse, this.game.camera)),
    );
    ...
    if (mouse.pressing && this.player) {
      ...
      if (d) {
        ...
        if (this.incoming > 0) {
          ...
          if (
            this.opponent.has("cannoneer") &&
            this.incoming > 0 &&
            this.hits < 2 &&
            this.canTake(this.incoming)
          ) {
            this.scene.info();
            setTimeout(() => {
              this.game.audio.incoming();
              this.scene.info(
                `Another ${this.incoming} damage!`,
                "Choose cargo taking the hit...",
              );
            }, 750);
            return;
          }
  ...
  public giveDice(amount: number): void {
    this.dice.slice(0, amount).forEach((d, i) => {
      ...
      d.move(
        offset(
          this.opponent.getDicePos(this.opponent.dice.length + i),
          this.opponent.p.x + 150,
          this.opponent.p.y,
        ),
        () => {
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.translate(
      this.p.x - this.stagger * 20 * mirror,
      this.p.y - Math.cos(this.animationPhase) * 10,
    );
    ...
    ctx.quadraticCurveTo(
      -200 - this.animationPhase * 10,
      -400,
      mastPos - 340 - this.animationPhase * 10,
      -180 - this.animationPhase * 10,
    );
  ...
  public repotionQuartermaster(): void {
    ...
    if (qm) {
      ...
      qm.hop({
        x: -45 - Math.max(Math.floor(this.dice.length / height), 1) * 40,
        y: -105 - Math.min(height, this.dice.length) * 70,
      });
    }
  }
  ...
  public sail(dir: number = 1): void {
    ...
    for (let i = 0; i < 7; i++) {
      setTimeout(
        () => this.game.audio.sail(0.1 + Math.random() * 0.3),
        i * 750,
      );
    }
  }
  ...
  public getCargoWidth(): number {
    return (
      Math.floor(
        Math.max(this.has("navigator") ? 4 : 1, this.dice.length - 1) /
          this.getStackHeight(),
      ) * 100
    );
  }
