...
export class Scene extends Container {
  ...
  constructor(game: Game) {
    ...
    this.secondLine = new WobblyText(game, "", 25, 400, 105, 0.2, 3, {
      shadow: 3,
      align: "center",
      scales: true,
    });
    this.bigText = new WobblyText(game, "~ COUP AHOO ~", 80, 400, 150, 0.2, 3, {
      shadow: 6,
      align: "center",
      scales: true,
    });
    this.action = new ButtonEntity(
      game,
      "ROLL",
      800 - 100 - 10,
      360,
    ...
    this.yesButton = new ButtonEntity(
      game,
      "",
      800 - 70 - 10,
      360,
    ...
    this.noButton = new ButtonEntity(
      game,
      "",
      800 - 70 * 3 - 10 * 2,
      360,
    ...
    this.clouds = Array.from(Array(50)).map((_, i) => ({
      x: -1000 + i * 200,
  ...
  private rollForDamage(): void {
    ...
    setTimeout(
      () =>
        this.promptForReroll(
          "Would you like to roll again?",
          `The total is ${this.getDamage()}...`,
          () => this.shoot(),
        ),
      500,
    );
  ...
  private rerollOrAct(first: string, second: string, after: () => void): void {
    if (this.current.has("navigator") && !this.extraRerollUsed) {
      ...
      this.promptForReroll(
        first,
        this.loot.length > 0 ? second : `The total is ${this.getDamage()}...`,
        after,
      );
      return;
    }
  ...
  public shoot(): void {
    ...
    if (
      ...
      this.current.opponent.getDiceCount() > 1
    ) {
  ...
  public nextTurn(): void {
    ...
    if (this.enemy.isDead() && !this.won) {
      ...
      setTimeout(() => {
        ...
        this.promptForReroll(
          "Victory! Nicely done!",
          "Would you like to reroll the loot?",
          () => {
            this.promptSail();
            this.showGreed();
          },
  ...
  private addLoot(): void {
    ...
    [...this.loot]
      .sort(randomSorter)
      .slice(0, this.getSpiceCount())
      .forEach((l) => l.makeSpice());
  }
  ...
  private getEnemyName(): string {
    ...
    return this.level % 2 == 0
      ? randomCell(["VND", "MRC", "GIT", "POO", "SIN", "CSS", "ASH", "CAP"])
      : (13 - (this.level - 1) * 0.5).toString();
  }
  ...
  private activateLevel(): void {
      ...
      switch (randomInt(0, 4 - (this.ship.getAvailableRole() ? 0 : 1))) {
        case 0: {
          ...
          setTimeout(() => {
            ...
            this.promptSail();
            this.info(
              "Ahoy mate! Interested in trade?",
              hasSpice
                ? "I'll give you fresh cargo for your spice..."
                : "Looks like you don't have any spice though...",
            );
            if (hasSpice) {
        ...
        case 1: {
          setTimeout(() => {
            ...
            this.promptAnswerWith(
              ...
              "KEEP",
              "Hello there mate!",
              "Would you like to reroll all your cargo?",
              () => {
                this.ship.rerollAll();
                this.thank();
              },
        ...
        case 2: {
          ...
          this.promptSail();
          this.info(
            "Free cargo floating in the drink!",
            "A vessel must have sunken here...",
          );
          this.addLoot();
          break;
        }
        case 3: {
          ...
          setTimeout(() => {
            ...
            this.enemy?.hop();
            this.promptAnswer(
              "Ahoy! I could plate one of your cargo!",
              "It'll only be able to receive 1 damage at a time....",
              () => {
                this.ship.addPlate();
                this.thank();
              },
        ...
        case 4: {
          ...
          setTimeout(() => {
            ...
            this.enemy?.hop();
            this.promptAnswer(
              `Oi! Want to hire this ${crew.crewRole}?`,
              crew.getRoleDescription(),
              () => {
    ...
    setTimeout(() => {
      this.info(
        "Man the cannons! Battle stations!",
        "There's no parley in sight...",
      );
      this.enemy.talk(
        randomCell([
          "FILTHY RAT",
  ...
  private moveDiceTo(ship: Ship): void {
    ...
    this.dice.forEach((d, i) =>
      d.move(
        offset(
          ship.getDicePos(i + ship.getDiceCount()),
          this.ship.p.x,
          this.ship.p.y,
        ),
        () => ship.addDice(d),
    ...
    for (let i = 0; i < amount; i++) {
      ...
      d.roll(
        m +
          offX +
          i * 120 -
          120 * (Math.min(amount - 1, perRow) * 0.5) -
          120 * perRow * Math.floor(i / perRow),
        450 + row * 120 + offY,
      );
      this.dice.push(d);
    }
  ...
  public update(tick: number, mouse: Mouse): void {
    ...
    if (Math.abs(z) > 0.01)
      this.cam.zoom += Math.sign(z) * 0.0075 * 0.05 * this.delta;
    ...
    if (this.loot.length > 0 && mouse.pressing) {
      ...
      if (looted) {
        ...
        looted.move(
          offset(
            this.ship.getDicePos(this.ship.getDiceCount()),
            this.ship.p.x,
            this.ship.p.y,
          ),
          () => this.ship.addDice(looted),
        );
        this.ship.openMouth();
        setTimeout(() => {
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    ...
    this.clouds.forEach((c) => {
      ...
      if (c.x < this.cam.pan.x - 1000)
        c.x = this.cam.pan.x + 2500 + Math.random() * 1000;
    });
    ...
    for (let i = 0; i < 6000 / 50; i++) {
      const top =
        Math.sin(start + i * 50 + 25 * this.wave) * 8 +
        Math.cos(start + i * 25 + 12 * this.fastWave) * 8 +
        20;
      ctx.quadraticCurveTo(
        start + i * 50 - 25,
        525 - this.animationPhaseAbs * 5 - top,
        start + i * 50,
        500 + this.animationPhaseAbs * 7 - top,
      );
    }
    ctx.fill();
    ctx.stroke();
    ...
    if (this.mp) {
      const p = new DOMPoint(this.mp.x, this.mp.y).matrixTransform(
    ...
    [this.splash, this.secondLine, this.bigText, ...this.getButtons()].forEach(
      (b) => b.draw(ctx),
    );
  }
}
