...
export const bits = (game: Game, p: Vector, color: string): RectParticle[] => {
  return Array.from(Array(10)).map(() => {
    ...
    return new RectParticle(
      ...
      size,
      0.3,
      { x: Math.cos(angle) * speed * 3, y: (Math.sin(angle) - 1) * speed },
      { force: { x: 0, y: 20000 }, color },
    );
  });
};
...
export const beams = (
  ...
  return Array.from(Array(amount)).map((_, i) => {
    ...
    return new LineParticle(
      game,
      p,
      offset(
        p,
        Math.cos(angle) * length * Math.random(),
        Math.sin(angle) * length * Math.random(),
      ),
      0.5,
      5 + 30 * Math.random(),
      "#fff",
